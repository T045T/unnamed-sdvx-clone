...
bool AudioPlayback::Init(class BeatmapPlayback& playback, const String& mapRootPath)
{
	...
	if (!audioPath.empty())
	{
		...
		{
			Logf("FX audio for for beatmap does not exists at: \"%s\" Using real-time effects instead.", Logger::Warning,
				audioPath);
		}
		else
		{
...
void AudioPlayback::m_SetLaserEffectParameter(float input)
{
	...
	case EffectType::PeakingFilter:
	{
		...
		bqfDSP->SetPeaking(m_laserEffect.peaking.q.Sample(input), m_laserEffect.peaking.freq.Sample(input),
							m_laserEffect.peaking.gain.Sample(input) * mix);
		break;
	}
