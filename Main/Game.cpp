...
// Try load map helper
shared_ptr<Beatmap> TryLoadMap(const String& path)
{
	// Load map file
	...
	return shared_ptr<Beatmap>(newMap);
}

Game::Game(const DifficultyIndex& difficulty)
{
	// Store path to map
...
Game::Game(const String& mapPath)
{
	// Store path to map
...
bool Game::AsyncFinalize()
{
	...
	g_input.OnButtonPressed.Add(this, &Game::m_OnButtonPressed);
...
void Game::OnKeyPressed(int32 key)
{
	if (key == SDLK_PAUSE)
	{
...
/**
 * \brief Initialize HUD elements/layout
...
bool Game::InitGameplay()
{
	...
	m_playback.OnEventChanged.Add(this, &Game::OnEventChanged);
	m_playback.OnLaneToggleChanged.Add(this, &Game::OnLaneToggleChanged);
	m_playback.OnFXBegin.Add(this, &Game::OnFXBegin);
	m_playback.OnFXEnd.Add(this, &Game::OnFXEnd);
	m_playback.OnLaserAlertEntered.AddLambda([=](LaserObjectState* object) {
	...
	if (m_usecMod)
	{
		m_playback.OnTimingPointChanged.AddLambda([=](TimingPoint* tp) {
			m_hispeed = m_modSpeed / tp->GetBPM();
		});
	}

	// Register input bindings
	m_scoring.OnButtonMiss.Add(this, &Game::OnButtonMiss);
	m_scoring.OnLaserSlamHit.Add(this, &Game::OnLaserSlamHit);
	m_scoring.OnButtonHit.Add(this, &Game::OnButtonHit);
	m_scoring.OnComboChanged.AddLambda([=](uint32 newCombo) {
		m_comboAnimation.Restart();
	});
	m_scoring.OnObjectHold.Add(this, &Game::OnObjectHold);
	m_scoring.OnObjectReleased.Add(this, &Game::OnObjectReleased);
	m_scoring.OnScoreChanged.AddLambda([=](uint32 newScore) {
		// Update score text
		if (m_scoreText)
...
/**
 * \brief Skips ahead to the right before the first object in the map
 */
bool Game::SkipIntro()
{
...
/**
 * \brief Processes input and Updates scoring, also handles audio timing management.
 */
void Game::TickGameplay(float deltaTime)
{
	if (!m_started)
	{
...
void Game::Restart()
{
	...
	for (auto& m_laserFollowEmitter : m_laserFollowEmitters)
	{
		if (m_laserFollowEmitter)
			m_laserFollowEmitter.reset();
	}

	for (auto& m_holdEmitter : m_holdEmitters)
	{
		if (m_holdEmitter)
			m_holdEmitter.reset();
	}
...
/**
 * \brief Called when game is finished and the score screen should show up.
 */
void Game::FinishGame()
{
	...
	transition->OnLoadingComplete.AddLambda([=](IAsyncLoadableApplicationTickable* tickable) {
...
shared_ptr<ParticleEmitter> Game::CreateTrailEmitter(const Color& color)
{
	shared_ptr<ParticleEmitter> emitter = m_particleSystem->add_emitter();
	...
	emitter->SetStartColor(PPConstant<Color>(static_cast<Color>(color * 0.7f)));
...
shared_ptr<ParticleEmitter> Game::CreateHoldEmitter(const Color& color, float width)
{
	shared_ptr<ParticleEmitter> emitter = m_particleSystem->add_emitter();
	emitter->material = particleMaterial;
	emitter->texture = basicParticleTexture;
	...
	emitter->SetStartColor(PPConstant<Color>(static_cast<Color>(color * 0.6f)));
...
shared_ptr<ParticleEmitter> Game::CreateExplosionEmitter(const Color& color, Vector3 dir)
{
	shared_ptr<ParticleEmitter> emitter = m_particleSystem->add_emitter();
	emitter->material = particleMaterial;
	emitter->texture = basicParticleTexture;
...
shared_ptr<ParticleEmitter> Game::CreateHitEmitter(const Color& color, float width)
{
	shared_ptr<ParticleEmitter> emitter = m_particleSystem->add_emitter();
	emitter->material = particleMaterial;
	emitter->texture = basicParticleTexture;
...
void Game::OnLaserSlamHit(LaserObjectState* object)
{
	const float slamSize = fabs(object->points[1] - object->points[0]);
	...
	const float dir = Math::Sign(object->points[1] - object->points[0]);
	const float laserPos = m_track->trackWidth * object->points[1] - m_track->trackWidth * 0.5f;
...
void Game::OnButtonHit(Input::Button button, ScoreHitRating rating, ObjectState* hitObject, bool late)
{
	const auto st = reinterpret_cast<ButtonObjectState*>(hitObject);
	const auto buttonIdx = static_cast<uint32>(button);
	Color c = m_track->hitColors[static_cast<size_t>(rating)];
	...
	if (rating != ScoreHitRating::Idle)
	{
		...
		const Color hitColor = (buttonIdx < 4) ? Color::White : Color::FromHSV(20, 0.7f, 1.0f);
		const float hitWidth = (buttonIdx < 4) ? Track::buttonWidth : Track::fxbuttonWidth;
...
void Game::OnButtonMiss(Input::Button button, bool hitEffect)
{
	const auto buttonIdx = static_cast<uint32>(button);
	if (hitEffect)
	{
...
/**
 * \brief controls if FX button DSP's are muted
 */
void Game::OnObjectHold(Input::Button, ObjectState* object)
{
	if (object->type == ObjectType::Hold)
	{
		auto hold = reinterpret_cast<HoldObjectState*>(object);
...
/**
 * \brief controls if FX button DSP's are not muted
 */
void Game::OnObjectReleased(Input::Button, ObjectState* object)
{
	if (object->type == ObjectType::Hold)
	{
		auto hold = reinterpret_cast<HoldObjectState*>(object);
...
void Game::OnLaneToggleChanged(LaneHideTogglePoint* tp)
{
...
void Game::OnEventChanged(EventKey key, EventData data)
{
	...
	else if (key == EventKey::TrackRollBehaviour)
	{
		...
		const auto i = static_cast<uint8>(data.rollVal) & 0x3;

		if (i == 0)
			m_rollIntensity = 0;
		else
			m_rollIntensity = m_rollIntensityBase + static_cast<float>(i - 1) * 0.0125f;
	}
...
void Game::m_OnButtonPressed(Input::Button buttonCode)
{
...
/**
 * \brief Wait before start of map.
 */
void Game::ApplyAudioLeadin()
{
...
bool Game::GetTickRate(int32& rate)
{
...
Texture Game::GetJacketImage()
{
	return m_jacketTexture;
}

shared_ptr<Beatmap> Game::GetBeatmap()
{
	return m_beatmap;
}

class Track& Game::GetTrack()
{
	return *m_track;
}

class Camera& Game::GetCamera()
{
	return m_camera;
}

class BeatmapPlayback& Game::GetPlayback()
{
	return m_playback;
}

class Scoring& Game::GetScoring()
{
	return m_scoring;
}

float* Game::GetGaugeSamples()
{
	return m_gaugeSamples;
}

const String& Game::GetMapRootPath() const
{
	return m_mapRootPath;
}

const String& Game::GetMapPath() const
{
	return m_mapPath;
}

const DifficultyIndex& Game::GetDifficultyIndex() const
{
	return m_diffIndex;
}