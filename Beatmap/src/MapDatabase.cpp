...
void MapDatabase::Update()
{
	...
	for (Event& e : changes)
	{
		if (e.action == Event::Added)
		{
			...
			const String mapPath = Path::RemoveLast(e.path, nullptr);
			bool existingUpdated;
			MapIndex* map;

			// Add or get map
			const auto mapIt = m_mapsByPath.find(mapPath);
		...
		else if (e.action == Event::Updated)
		{
			...
			const auto itMap = m_maps.find(itDiff->second->mapId);
...
{
	MapIndex** mapIdx = m_maps.Find(idx);
	return mapIdx ? *mapIdx : nullptr;
}

void MapDatabase::AddSearchPath(const String& path)
{
	const String normalizedPath = Path::Normalize(Path::Absolute(path));
...
void MapDatabase::m_SearchThread()
{
...

	{
		...
		for (const String& rootSearchPath : m_searchPaths)
		{
...

	{
		...
		for (const auto& f : m_searchState.difficulties)
		{
...
/**
 * \brief Thread safe event queue functions. Add a new change to the change queue.
 */
...
/**
 * \brief Removes changes from the queue and returns them.
 * \param maxChanges maximum amount of changes to remove from the queue
 */
List<MapDatabase::Event> MapDatabase::FlushChanges(size_t maxChanges)
{
	List<Event> changes;
	m_pendingChangesLock.lock();
	if (maxChanges == -1)
