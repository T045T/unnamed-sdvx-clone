...
namespace VectorMath
{
	...
	public:
		...
		template <typename T1>
		VectorBase(const VectorBase<T1, 3>& other, T w = 0)
			: w(w)
		{
			x = static_cast<T>(other.x);
			y = static_cast<T>(other.y);
		...
		template <typename T1>
		VectorBase(const VectorBase<T1, 2>& other, T z = 0, T w = 0)
			: z(z), w(w)
		{
			x = static_cast<T>(other.x);
			y = static_cast<T>(other.y);
	...
	public:
		...
		template <typename T1>
		VectorBase(const VectorBase<T1, 2>& other, T z = 0)
			: z(z)
		{
			x = static_cast<T>(other.x);
			y = static_cast<T>(other.y);
